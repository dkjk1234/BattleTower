...
public class Monster : MonoBehaviour
{
    ...
    {
        if (LevelManager.Instance.MapPaths[wayIndex] != null) SetPath(LevelManager.Instance.MapPaths[wayIndex].Path, wayIndex);
        else
        {
            TakeDamage(10000, Element.WIZARD);
        }
    ...
    public void ApplyPoison(float damagePerTick, float duration, float tickInterval)
    {
        ...
        poisonCoroutine = StartCoroutine(PoisonCoroutine(damagePerTick, duration, tickInterval));
    }
    ...
    private IEnumerator PoisonCoroutine(float damagePerTick, float duration, float tickInterval)
    {
        ...
        float tickTimer = 0f;
        originalColor = spriteRenderer.color;
        spriteRenderer.color = Color.green; // Change color to green for poison
        ...
        while (elapsed < duration && currentHealth > 0)
        {
            ...
            {
                tickTimer = 0f;
                TakeDamage(damagePerTick, Element.WIZARD);
            }
            yield return null;
        }
